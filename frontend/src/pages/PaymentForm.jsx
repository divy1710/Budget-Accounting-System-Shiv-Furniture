  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const res =
          contactType === "VENDOR"
            ? await contactsApi.getVendors()
            : await contactsApi.getCustomers();
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              >
                <option value="">
                  Select {contactType === "VENDOR" ? "Vendor" : "Customer"}
                </option>
                {contacts.map((c) => (
        ...
        {formData.contactId && outstanding.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">
              Allocate to Outstanding{" "}
              {contactType === "VENDOR" ? "Bills" : "Invoices"}
            </h2>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase w-40">
                      Allocate
                    </th>
                  </tr>
                </thead>
                      ...
                      <tr key={txn.id}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <input
                            ...
                            onChange={(e) =>
                              handleAllocationChange(txn.id, e.target.value)
                            }
            ...
            <div className="mt-4 flex justify-end">
              <div className="w-64 space-y-2">
                ...
                <div
                  className={`flex justify-between font-bold border-t pt-2 ${unallocated < 0 ? "text-red-600" : "text-gray-800"}`}
                >
        ...
        {formData.contactId && outstanding.length === 0 && (
          <div className="bg-white rounded-xl shadow-sm p-6 text-center text-gray-500">
            No outstanding {contactType === "VENDOR" ? "bills" : "invoices"} for
            this {contactType === "VENDOR" ? "vendor" : "customer"}
          </div>
        )}

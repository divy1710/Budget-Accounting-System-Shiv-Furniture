...
export default function TransactionList({ type, title }) {
  ...
  const handleConfirm = async (id) => {
    if (
      window.confirm(
        "Confirm this transaction? Budget will be updated accordingly.",
      )
    ) {
      try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => navigate(`/transactions/new?type=${type}`)}
                ...
                {filteredTransactions.map((txn) => (
                  <tr key={txn.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-gray-700">
                      {isVendorType ? txn.vendor?.name : txn.customer?.name}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span
                        className={`inline-flex px-2 py-1 text-xs rounded-full ${getStatusColor(txn.status)}`}
                      >
                        {txn.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span
                        className={`inline-flex px-2 py-1 text-xs rounded-full ${getPaymentStatusColor(txn.paymentStatus)}`}
                      >

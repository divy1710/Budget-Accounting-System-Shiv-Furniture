          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gradient-to-r from-gray-700 to-gray-600">
                <th className="px-4 py-3 text-left text-white font-bold border-r border-gray-600">Payment No.</th>
                <th className="px-4 py-3 text-left text-white font-bold border-r border-gray-600">Type</th>
                <th className="px-4 py-3 text-left text-white font-bold border-r border-gray-600">Partner</th>
                <th className="px-4 py-3 text-left text-white font-bold border-r border-gray-600">Date</th>
                <th className="px-4 py-3 text-left text-white font-bold border-r border-gray-600">Method</th>
                <th className="px-4 py-3 text-right text-white font-bold border-r border-gray-600">Amount</th>
            ...
            <tbody>
              {payments.map((payment, index) => (
                ...
                >
                  <td className="px-4 py-3 text-white font-medium border-r border-gray-700">{payment.paymentNumber}</td>
                  ...
                  <td className="px-4 py-3 text-white border-r border-gray-700">{payment.contact?.name || "-"}</td>
                  <td className="px-4 py-3 text-white border-r border-gray-700">
                    {new Date(payment.paymentDate).toLocaleDateString()}
                  </td>
                  <td className="px-4 py-3 text-cyan-400 capitalize border-r border-gray-700">{payment.paymentMethod}</td>
              ...
              {payments.length === 0 && (
                <tr>
                  <td colSpan="7" className="px-4 py-12 text-center text-gray-400 bg-gray-800">
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-gray-900 rounded-lg p-4 border border-gray-700">
        <h1 className="text-2xl font-bold text-white">
          {selectedPayment ? `✏️ Edit: ${selectedPayment.paymentNumber}` : "💳 New Bill Payment"}
        </h1>
        <div className="flex gap-3">
          <button
            onClick={() => navigate("/")}
      ...
      <form onSubmit={handleSubmit} className="bg-gradient-to-b from-gray-900 to-gray-800 rounded-xl border border-gray-700 shadow-2xl">
        ...
        <div className="p-6 border-b border-gray-700">
          <label className="block text-gray-400 text-sm font-medium uppercase tracking-wide mb-4">Payment Type</label>
          <div className="flex gap-6">
            <label className={`flex items-center gap-3 cursor-pointer px-6 py-4 rounded-xl border-2 transition-all ${
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, paymentType: e.target.value, contactId: "" })}
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="block text-gray-400 text-sm font-medium uppercase tracking-wide">Partner</label>
              <select
                ...
                required
              >
                <option value="">Select {formData.paymentType === "SEND" ? "Vendor" : "Customer"}</option>
              ...
              <span className="text-xs text-gray-500">auto fill partner name from Invoice/Bill</span>
            </div>

            {/* Date */}
            <div className="space-y-2">
              <label className="block text-gray-400 text-sm font-medium uppercase tracking-wide">Date</label>
              <input
                type="date"
                ...
                required
              />
              <span className="text-xs text-gray-500">(Default Today Date)</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            {/* Amount */}
            <div className="space-y-2">
              <label className="block text-gray-400 text-sm font-medium uppercase tracking-wide">Amount</label>
              <input
                type="number"
                ...
                required
              />
              <span className="text-xs text-gray-500">(auto fill amount due from Invoice/Bill)</span>
            </div>

            {/* Payment Via */}
            <div className="space-y-2">
              <label className="block text-gray-400 text-sm font-medium uppercase tracking-wide">Payment Via</label>
              <select
              ...
              <span className="text-xs text-gray-500">(Default Bank can be selectable to Cash)</span>
            </div>
          </div>
          ...
          {/* Note */}
          <div className="space-y-2">
            <label className="block text-gray-400 text-sm font-medium uppercase tracking-wide">Note</label>
            <textarea
        ...
        <div className="bg-gradient-to-b from-gray-900 to-gray-800 rounded-xl border border-gray-700 shadow-2xl">
          ...
          <div className="p-6 border-b border-gray-700">
            <div className="flex items-center gap-4">
              <span className="text-gray-400 text-sm uppercase tracking-wide">Payment No.</span>
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-6">
                <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                  <label className="text-gray-400 text-sm uppercase tracking-wide">Payment Type</label>
                  <div className="flex gap-6 mt-3">
                    <span className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 ${
                ...
                <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                  <label className="text-gray-400 text-sm uppercase tracking-wide">Partner</label>
                  <p className="text-white text-xl font-medium mt-2">{selectedPayment.contact?.name || "-"}</p>
                </div>

                <div className="bg-gradient-to-r from-cyan-900/30 to-blue-900/30 p-5 rounded-xl border-2 border-cyan-500">
                  <label className="text-gray-400 text-sm uppercase tracking-wide">Amount</label>
              ...
              <div className="space-y-6">
                <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                  <label className="text-gray-400 text-sm uppercase tracking-wide">Date</label>
                ...
                <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                  <label className="text-gray-400 text-sm uppercase tracking-wide">Payment Via</label>
                  <p className="text-white text-lg font-medium mt-2 capitalize">{selectedPayment.paymentMethod}</p>
                </div>

                <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                  <label className="text-gray-400 text-sm uppercase tracking-wide">Reference</label>
                  <p className="text-cyan-400 text-lg font-medium mt-2">{selectedPayment.reference || "-"}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {selectedPayment.notes && (
            <div className="mx-6 mb-6 bg-gray-800/50 p-4 rounded-xl border border-gray-700">
              <label className="text-gray-400 text-sm uppercase tracking-wide">Note</label>
          ...
          {selectedPayment.allocations?.length > 0 && (
            <div className="mx-6 mb-6">
              <label className="text-gray-400 text-sm uppercase tracking-wide block mb-3">Applied to</label>
              <div className="border-2 border-gray-600 rounded-xl overflow-hidden">
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-gray-700 to-gray-600">
                      <th className="px-4 py-3 text-left text-white font-bold border-r border-gray-600">Transaction</th>
                      <th className="px-4 py-3 text-right text-white font-bold">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {selectedPayment.allocations.map((alloc, index) => (
                      <tr key={alloc.id} className={`border-t-2 border-gray-600 ${
                        index % 2 === 0 ? "bg-gray-800" : "bg-gray-800/50"
                      }`}>
                        <td className="px-4 py-3 border-r border-gray-700">
                          <button
